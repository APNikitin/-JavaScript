...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && !havePoison(new_unit)) {
       ...
       if (!haveFood(new_unit)) {
        ...
        document.getElementById("score_field").innerHTML=score;
...
function havePoison(unit) {
    ...
    // Если яд
    if (unit_classes.includes('poison-unit')) {
        check = true;
...
function createPoison() {
    ...
    while (!poisonCreated) { //пока яд не создали
        ...
        var poison_cell = document.getElementsByClassName('cell-' + poison_y + '-' + poison_x)[0];
        ...
        if (!poison_cell_classes.includes('snake-unit')) {
            ...
            poison_cell.setAttribute('class', classes + 'poison-unit');
            poisonCreated = true;
        }
    }
}
